...
function Header() {
    ...
    return (
        <>
        <header>
            <nav className="flex justify-between items-center fixed w-full  p-4 pb-0 shadow-sm shadow-[0_1px_5px_0_hsla(0, 0%, 0%, 0.5)] z-10 backdrop-blur-sm">
                    <img src={logo} alt="arofah-logo" className="w-[5rem]" />
            
            {/* navbar links */}
                <ul className="absolute w-full top-[100%] right-0 text-brand-600 pl-4 pb-8 pt-4 shadow-md shadow-brand-400 rounded-md transition duration-500 ease-in-out z-10 bg-brand-100 translate-x-[30rem] h-[100vh] md:hidden" ref={linkBoxRef}>
                ...
                {/* navbar button */}
                <div className="cursor-pointer absolute top-8 right-10 lg:hidden" onClick={()=>{
                    ...
                    linkBoxRef.current.classList.toggle('translate-x-[30rem]');
                    }}>
